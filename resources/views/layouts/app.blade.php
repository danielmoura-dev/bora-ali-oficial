    ...
    <header class="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div class="max-w-6xl mx-auto px-4">
            ...
            <div class="flex items-center justify-between h-14">
                <a href="{{ route('home') }}"
                   class="text-xl font-bold text-indigo-600 shrink-0">
                    Bora Ali 🎉
                </a>

                {{-- Desktop: busca central --}}
                <form method="GET" action="{{ route('home') }}"
                      class="hidden md:flex flex-1 max-w-md mx-6">
                    ...
                    @auth
                        ...
                        <div class="relative ml-2" x-data="{ open: false }">
                            <button @click="open = !open"
                                ...
                                @else
                                    <div class="w-7 h-7 rounded-full bg-indigo-100 flex items-center
            ...
            <div id="mobile-menu" class="md:hidden hidden border-t border-gray-100 py-3 space-y-1">
                @auth
                    ...
                    <form method="POST" action="{{ route('auth.logout') }}" class="px-3">
                        @csrf
                        <button type="submit"
                                class="w-full text-left py-2 text-sm text-red-600">
                            Sair
                        </button>
                    </form>
                @else
