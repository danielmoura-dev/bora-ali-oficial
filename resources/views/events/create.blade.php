        ...
        <form method="POST" action="{{ route('events.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
                ...
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="is_free" id="is_free" value="1"
                        {{ old('is_free') ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded border-gray-300">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="venue_name" value="{{ old('venue_name') }}" placeholder="Ex: Arena Castelão"
                        class="w-full px-4 py-3 rounded-xl border border-gray-200
                              focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <label
                            class="flex items-start gap-3 p-4 border-2 border-gray-100
                          rounded-xl cursor-pointer hover:border-indigo-300
                          has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50
                          transition">
                            <input type="radio" name="payment_mode" value="direct" class="mt-0.5 text-indigo-600"
                                checked>
                            <div>
                        ...
                        <label
                          ...
                          has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50
                          transition">
                            <input type="radio" name="payment_mode" value="split" class="mt-0.5 text-indigo-600">
                            <div>
                                <p class="font-medium text-sm text-gray-800">
                                    Split automático
                                </p>
                ...
                <div id="split-warning"
                    class="hidden p-3 bg-yellow-50 border border-yellow-200
                                    rounded-xl text-xs text-yellow-800">
                    Para usar o split automático, você precisa conectar sua conta do
                    Mercado Pago em
                    <a href="{{ route('profile.show') }}" class="underline font-medium">
                        Meu Perfil
                    </a>.
                </div>
            </div>
            ...
            <script>
                document.querySelectorAll('input[name="payment_mode"]').forEach(radio => {
                    radio.addEventListener('change', function() {
                        ...
                        warning.classList.toggle('hidden', this.value !== 'split');
                    });
                });
            </script>
            ...
            <div class="flex items-center justify-between gap-3">
                <a href="{{ route('home') }}" class="px-6 py-3 text-sm text-gray-600 hover:text-gray-800 transition">
                    Cancelar
                </a>
                <button type="submit"
